...
const Page: PageEl = (props, refresh, getProps, dies, z): React.JSX.Element => {
    ...
    getProps(async () => { })
    let empty = true;
    let styles = z.qestyles
    return <div><Window title={props.title} style={{}}>
        <div className={z.qestyles.gridcontainer}>

            {!props.single || props.selected.length == 0 ? <div className={z.qestyles.griditem} style={{
            ...
            {props.selected?.map(s => {
                return <div className={z.qestyles.griditem + " " + z.qestyles.hover} key={s.name}
                    style={{ backgroundColor: "#b5c9b0aa", borderRadius: 3, position: "relative", transition: "all linear 0.1s" }}
                    onClick={() => {
                        props.onitemclick?.(s)
                    }}>
                    <img className={z.qestyles.hover} src={global.cdn("/files/close.svg")} style={{
                    ...
                    <f-cc onClick={(e) => {
                        ...
                        <img src={global.cdn(s.icon)} style={{
                            ...
                            marginLeft: z.lang.dir == "rtl" ? 5 : 0,
                            marginRight: z.lang.dir == "ltr" ? 5 : 0
                        }} />
                        <f-11>{TextEndAbbreviation(s.name || z.lang.loading, 30)}</f-11><sp-2 />
                    </f-cc>
                </div>
            })}
        ...
        {props.form == "selected" ? <WindowFloat title={props.title}
            style={{ maxHeight: 400, maxWidth: 400 }} onclose={() => { props.form = null; refresh() }}>
